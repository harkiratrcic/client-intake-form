...
export default async function FormsPage({ searchParams }: FormsPageProps) {
  ...
  const pageNum = parseInt(searchParamsResolved.page || '1', 10);
  const page = isNaN(pageNum) || pageNum < 1 ? 1 : pageNum;
  const search = searchParamsResolved.search || '';
  const status = searchParamsResolved.status as 'SENT' | 'IN_PROGRESS' | 'COMPLETED' | 'EXPIRED' | undefined;
  const sortBy = searchParamsResolved.sortBy as 'createdAt' | 'updatedAt' | 'clientEmail' | 'status' | 'expiresAt' | undefined;
  const sortOrder = searchParamsResolved.sortOrder as 'asc' | 'desc' | undefined;
